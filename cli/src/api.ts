...
import { Whistlepoddu } from '@midnight-whistlepoddu/contract';
...
import {
  type WhistlepodduCircuits,
  type WhistlepodduContract,
  type WhistlepodduPrivateStateId,
  type WhistlepodduProviders,
  type DeployedWhistlepodduContract,
...
const whistlepodduCompiledContract = CompiledContract.make('whistlepoddu', Whistlepoddu.Contract).pipe(
...
): Promise<DeployedWhistlepodduContract> => {
  const whistlepodduContract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: whistlepodduCompiledContract,
    privateStateId: 'whistlepodduPrivateState',
    initialPrivateState: {},
  });
  logger.info(`Joined Whistlepoddu contract at address: ${whistlepodduContract.deployTxData.public.contractAddress}`);
  return whistlepodduContract as any;
};

export const deploy = async (
  providers: WhistlepodduProviders,
): Promise<DeployedWhistlepodduContract> => {
  logger.info('Deploying Whistlepoddu contract...');
  const whistlepodduContract = await deployContract(providers, {
    compiledContract: whistlepodduCompiledContract,
    privateStateId: 'whistlepodduPrivateState',
    initialPrivateState: {},
  });
  const address = whistlepodduContract.deployTxData.public.contractAddress;
  logger.info(`Deployed Whistlepoddu contract at address: ${address}`);
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof WhistlepodduPrivateStateId>({
      privateStateStoreName: 'whistlepoddu-cli-storage-v705',
...
export type { WhistlepodduProviders };
